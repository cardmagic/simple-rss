...
class BaseTest < Test::Unit::TestCase
  def setup
    ...
    @video_rss = SimpleRSS.parse open(File.dirname(__FILE__) + '/../data/media_rss_with_video.xml')
  ...
  def test_media_rss_with_video
    assert_equal 20, @video_rss.items.size
    assert_equal 'Uploads from herval', @video_rss.title
    assert_equal 'http://www.flickr.com/photos/herval/', @video_rss.channel.link
    assert_equal 'http://www.flickr.com/photos/herval/4671960608/', @video_rss.items.first.link
    assert_equal 'http://www.flickr.com/photos/herval/4671960608/', @video_rss.items.first[:link]
    assert_equal 'http://example.com/woof.mp4', @video_rss.items.first.media_content_url
    assert_equal 'video/mp4', @video_rss.items.first.media_content_type
    assert_equal '65', @video_rss.items.first.media_content_duration
    assert_equal 'Woof?', @video_rss.items.first.media_title
    assert_equal 'http://farm5.static.flickr.com/4040/4671960608_954d2297bc_s.jpg', @video_rss.items.first.media_thumbnail_url
    assert_equal '75', @video_rss.items.first.media_thumbnail_height
    assert_equal '75', @video_rss.items.first.media_thumbnail_width
    assert_equal 'herval', @video_rss.items.first.media_credit
    assert_equal 'photographer', @video_rss.items.first.media_credit_role
    assert_equal 'pets frodo', @video_rss.items.first.media_category
  end
